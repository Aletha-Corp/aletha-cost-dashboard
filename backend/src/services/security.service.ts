...
let reportCache: { data: SecurityReport; cachedAt: number } | null = null;
let inFlightPromise: Promise<SecurityReport> | null = null;
...
function buildSummary(findings: SecurityFinding[]): SecurityScanSummary {
  ...
  try {
    ...
    if (admins.length > 0) {
      findings.push({
        ...
        description:
          `${admins.length} Co-Administrator(s) detected. Classic admin roles bypass Azure RBAC and cannot benefit from Conditional Access or PIM.`,
    ...
    logger.warn('checkClassicAdmins skipped', { err: String(err) });
  }
  return findings;
}
...
async function checkSubscriptionOwners(
  authClient: AuthorizationManagementClient,
  subscriptionId: string,
): Promise<SecurityFinding[]> {
  const findings: SecurityFinding[] = [];
  try {
    ...
    logger.warn('checkSubscriptionOwners skipped', { err: String(err) });
  }
  return findings;
}
...
async function checkNsgOpenPorts(
  networkClient: NetworkManagementClient,
): Promise<SecurityFinding[]> {
  const findings: SecurityFinding[] = [];
  try {
    const wideOpenNsgs: AffectedResource[] = [];
    const sshRdpNsgs: AffectedResource[] = [];
    ...
    logger.warn('checkNsgOpenPorts skipped', { err: String(err) });
  }
  return findings;
}
...
async function checkStorageSecurity(
  storageClient: StorageManagementClient,
): Promise<SecurityFinding[]> {
  const findings: SecurityFinding[] = [];
  try {
    const publicBlobAccounts: AffectedResource[] = [];
    const httpOnlyAccounts: AffectedResource[] = [];
    const noSoftDeleteAccounts: AffectedResource[] = [];
    ...
    logger.warn('checkStorageSecurity skipped', { err: String(err) });
  }
  return findings;
}
...
async function checkKeyVaults(
  kvClient: KeyVaultManagementClient,
): Promise<SecurityFinding[]> {
  const findings: SecurityFinding[] = [];
  try {
    const noSoftDeleteKvs: AffectedResource[] = [];
    const noPurgeProtectionKvs: AffectedResource[] = [];
    const publicNetworkKvs: AffectedResource[] = [];
    ...
    for await (const kv of kvClient.vaults.list()) {
      ...
      const props = kv as { properties?: { enableSoftDelete?: boolean; enablePurgeProtection?: boolean; publicNetworkAccess?: string; networkAcls?: { defaultAction?: string } } };
      if (props.properties?.enableSoftDelete === false) noSoftDeleteKvs.push(resource);
      if (!props.properties?.enablePurgeProtection) noPurgeProtectionKvs.push(resource);
    ...
    logger.warn('checkKeyVaults skipped', { err: String(err) });
  }
  return findings;
}
...
async function checkGovernance(
  ...
  const findings: SecurityFinding[] = [];
  try {
    const untaggedRgs: AffectedResource[] = [];
    const lockedRgs: Set<string> = new Set();

    // List all management locks at subscription scope
    for await (const rg of resourceClient.resourceGroups.list()) {
      const name = rg.name ?? '';
      const tags = rg.tags ?? {};
      const hasOwner = Object.keys(tags).some((k) =>
        ['owner', 'createdby', 'created_by', 'team'].includes(k.toLowerCase()),
      );
      const hasEnv = Object.keys(tags).some((k) =>
        ['environment', 'env', 'stage'].includes(k.toLowerCase()),
      );
      if (!hasOwner || !hasEnv) {
        untaggedRgs.push({
          id: rg.id ?? '',
    ...
    logger.warn('checkGovernance skipped', { err: String(err) });
  }
  return findings;
}
...
async function checkMonitoringLogging(
  resourceClient: ResourceManagementClient,
  subscriptionId: string,
): Promise<SecurityFinding[]> {
  const findings: SecurityFinding[] = [];
  try {
    ...
    for await (const resource of resourceClient.resources.list()) {
      ...
      if (type.includes('microsoft.operationalinsights/workspaces')) hasLogAnalytics = true;
      if (type.includes('microsoft.insights/diagnosticsettings')) hasStorageDiag = true;
    }
    ...
    logger.warn('checkMonitoringLogging skipped', { err: String(err) });
  }
  return findings;
}
...
async function checkDdosProtection(
  ...
  const findings: SecurityFinding[] = [];
  try {
    const unprotectedVnets: AffectedResource[] = [];
    for await (const vnet of networkClient.virtualNetworks.listAll()) {
      const ddos = (vnet as { enableDdosProtection?: boolean }).enableDdosProtection;
      if (!ddos) {
    ...
    logger.warn('checkDdosProtection skipped', { err: String(err) });
  }
  return findings;
}
...
async function doRunSecurityScan(subscriptionId: string): Promise<SecurityReport> {
  const startTime = Date.now();
  const credential = getAzureCredential();
  const skippedCategories: SecurityReport['skippedCategories'] = [];
  ...
  type CheckResult = SecurityFinding[] | { category: SecurityCategory; reason: string };
  ...
  const safeRun = async (
    ...
    try {
      ...
      logger.warn(`Security check category skipped: ${category}`, { reason });
      return [];
    }
  };
 ...
 */
export async function runSecurityScan(subscriptionId?: string): Promise<SecurityReport> {
  const subId = subscriptionId ?? env.AZURE_SUBSCRIPTION_ID;

  if (reportCache && Date.now() - reportCache.cachedAt < CACHE_TTL_MS) {
    logger.info('Returning security report from cache');
    return reportCache.data;
  }

  if (inFlightPromise) {
    logger.info('Coalescing into in-flight security scan');
...
export function invalidateSecurityCache(): void {
  reportCache = null;
  logger.info('Security report cache invalidated');
}
